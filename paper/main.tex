...
\textbf{\texttt{flow\_coupled}} (paper-scale, this paper) & \textbf{0.477} & \textbf{0.008} & $4.9 \times 10^{-7}$ & \textbf{$-69\%$} \\
\midrule
...
\item \textbf{Coupling concentrates the pixel-space depth distribution.} See \S\,pixel-space evaluation and Table~\ref{tab:pixel_eval} below: while token-space $\scloss$ drops cleanly, the variance of the decoded-depth distribution also drops, and the variant moves further from the real depth distribution in Wasserstein. We discuss this as the predictability-vs-fidelity trade-off.
\end{enumerate}
